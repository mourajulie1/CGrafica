...
g2 = 1
g3 = 2
cores = ( (1,0,0.5),(1,1,0),(0.5,1,0),(0.5,1,1),(0,0.5,1),(1,0.5,1),(0.8,0.5,0.5),(0.5,0,0.4) )
def prismaIluminado():
    glPushMatrix()
    ...
    for j in range(lados):
        x1 = raio * math.cos(j*ang)
        y1 = raio * math.sin(j*ang)
        base += [ (x1,y1) ]
    ...
    glBegin(GL_QUADS)
    for i in range(lados):
        a = (base[i][0],base[i][1],altura)
        b = (base[i][0],base[i][1],0.0)
        c = (base[(i+1)%lados][0],base[(i+1)%lados][1],0.0)
        d = (base[(i+1)%lados][0],base[(i+1)%lados][1],altura)
...
def principal():
    ...
    glutCreateWindow("iluminando o Prisma")
    initin()
